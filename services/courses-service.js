...
export default {
  ...
  getCoursesByInstructor: async(IDInstructor)=>{
    const result = await db('courses').leftJoin('discount','courses.IDDiscount','=','discount.ID').where({IDInstructor}).select('courses.ID','Name','TinyDesc', 'CourseFee', 'IsCompleted', 'PercentDiscount');
    return result;
  },
