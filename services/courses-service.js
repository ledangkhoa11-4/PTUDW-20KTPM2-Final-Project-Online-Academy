...
export default {
  ...
  getTotalCourseByStudentID: async (id) => {
    const total = await db.raw(`SELECT COUNT(*) AS count FROM participate LEFT JOIN courses ON participate.IDCourse = courses.ID WHERE courses.disable = 0 AND participate.IDStudent = ${id}`)
    return total[0][0].count;
  },
  getTotalCoursesByStudentIDWatchlist: async (id) => {
    const total = await db.raw(`SELECT COUNT(*) AS count FROM watchlist LEFT JOIN courses ON courses.ID = watchlist.IDCourse WHERE watchlist.IDStudent = ${id} AND courses.disable = 0;`)
    return total[0][0].count;
  }
};
